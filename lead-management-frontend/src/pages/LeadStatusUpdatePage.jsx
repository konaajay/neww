...
const LeadStatusUpdatePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await leadsApi.updateStatus(id, payload);
  ...
  return (
    <div className={`p-2 p-md-3 ${isDarkMode ? 'bg-black text-white' : 'bg-light text-dark'}`} style={{ minHeight: '100vh', display: 'block', position: 'relative' }}>
          ...
          <div className="p-3">
            <form onSubmit={handleSubmit}>
              <div className="d-flex flex-column gap-3">
                ...
                <div className={`p-3 rounded-4 border ${isDarkMode ? 'bg-surface bg-opacity-40 border-white border-opacity-5' : 'bg-light border-secondary border-opacity-10'}`}>
                   <label className="text-muted small fw-black text-uppercase tracking-widest mb-1 d-block" style={{ fontSize: '9px' }}>Current Protocol</label>
                ...
                {['CONVERTED', 'PAID', 'EMI'].includes(selectedStatus?.toUpperCase()) && (
                  <div className={`p-3 rounded-4 border border-secondary border-opacity-10 shadow-sm ${isDarkMode ? 'bg-surface' : 'bg-white'}`}>
                    ...
                    <div className="row g-2 mb-4">
                      <div className="col-6">
                        <button type="button" onClick={() => setPaymentType('FULL')} className={`btn w-100 py-3 fw-bold rounded-3 ${paymentType === 'FULL' ? 'btn-primary' : 'btn-outline-secondary opacity-50'}`}>FULL SETTLEMENT</button>
                      </div>
                      <div className="col-6">
                      ...
                      <div className="p-3 rounded-4 bg-light bg-opacity-5 border border-primary border-opacity-10 mb-4">
                        ...
                        {installments.map((inst, idx) => (
                          <div key={idx} className="d-flex gap-2 mb-2">
                             ...
                             <input type="datetime-local" className="form-control form-control-sm rounded-3" value={inst.dueDate} onChange={e => handleInstallmentChange(idx, 'dueDate', e.target.value)} />
                    ...
                    <div className={`p-3 rounded-4 text-center fw-black text-uppercase border mb-3 ${isMatch ? 'text-success border-success bg-success bg-opacity-5' : 'text-danger border-danger bg-danger bg-opacity-5'}`} style={{ fontSize: '11px' }}>
                       {isMatch ? `ACCOUNTING VERIFIED: ₹${sumOfParts.toLocaleString()}` : `MISMATCH: Commitment must match Settlement (₹${discountedTotal.toLocaleString()})`}
                    </div>
                  </div>
                )}
                ...
                {isManualPayment && (
                  <div className="p-4 rounded-4 border border-warning border-opacity-20 bg-warning bg-opacity-5">
                    <h6 className="fw-black text-uppercase tracking-wider mb-4 d-flex align-items-center gap-2">
                      <Shield size={18} className="text-warning" /> MANUAL PAYMENT VERIFICATION
                    </h6>
                    <PaymentOcrUpload 
                ...
                <button 
                  ...
                  className="w-100 py-3 ui-btn ui-btn-primary rounded-pill fw-black text-uppercase tracking-widest shadow-glow"
                >
