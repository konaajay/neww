...
const CourseManagementPage = () => {
    ...
    return (
        <div className={`min-vh-100 p-4 ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
            <div className="max-w-6xl mx-auto animate-fade-in">
                {/* Header Section */}
                <div className="d-flex align-items-center justify-content-between mb-5">
                    <div className="d-flex align-items-center gap-4">
                        <div className={`p-3 rounded-4 shadow-glow ${isDarkMode ? 'bg-blue-600 bg-opacity-20 text-blue-400 border border-blue-500 border-opacity-20' : 'bg-blue-600 text-white'}`}>
                            <BookOpen size={32} strokeWidth={1.5} />
                        </div>
                        <div>
                            <h2 className="fw-black text-uppercase tracking-widest mb-1" style={{ letterSpacing: '4px' }}>Strategic Course Hub</h2>
                    ...
                    {!isAdding && (
                        ...
                        >
                            <Plus size={18} />
                            <span className="fw-black text-uppercase" style={{ fontSize: '11px', letterSpacing: '2px' }}>New Protocol</span>
                        </button>
                    )}
                </div>
                ...
                {/* Adding/Editing Form */}
                {isAdding && (
                    <div className={`mb-5 p-4 rounded-5 border ${isDarkMode ? 'bg-slate-900 border-white border-opacity-10 shadow-2xl' : 'bg-white border-slate-200 shadow-xl'}`}>
                        <div className="d-flex align-items-center justify-content-between mb-4">
                            <h5 className="fw-black text-uppercase tracking-widest mb-0">{editingId ? 'Modify Protocol' : 'Deploy New Course'}</h5>
                        ...
                        <form onSubmit={handleSave} className="row g-4">
                            <div className="col-md-6">
                                ...
                                <input 
                                    className={`form-control ${isDarkMode ? 'bg-slate-800 border-white border-opacity-10 text-white' : 'bg-slate-50'} py-3 px-3 rounded-4 shadow-none border-0`}
                ...
                <div className="row g-4">
                    {courses.map(course => (
                        <div key={course.id} className="col-md-6 col-lg-4 animate-slide-up">
                            <div className={`h-100 p-4 rounded-5 border transition-all duration-300 hover-shadow-glow ${isDarkMode ? 'bg-slate-900 border-white border-opacity-5 hover:bg-slate-800' : 'bg-white border-slate-200 hover:bg-slate-50'}`}>
                                <div className="d-flex align-items-start justify-content-between mb-3">
                                    <div className={`p-2 rounded-3 ${isDarkMode ? 'bg-blue-600 bg-opacity-10 text-blue-400' : 'bg-blue-100 text-blue-600'}`}>
                                        <ShieldCheck size={20} />
                                    </div>
                                ...
                                <h5 className="fw-black text-uppercase tracking-widest mb-2" style={{ letterSpacing: '1px' }}>{course.name}</h5>
                                <p className="text-muted small mb-4 line-clamp-2" style={{ fontSize: '11px', height: '32px' }}>{course.description || 'No strategic description provided for this protocol.'}</p>
                                ...
                                <div className="d-flex align-items-center justify-content-between pt-3 border-top border-white border-opacity-5">
                                    <div>
                                        ...
                                        <p className="fw-black text-primary mb-0" style={{ fontSize: '18px' }}>₹{course.baseFee.toLocaleString()}</p>
                                    </div>
                                    <div className="text-end">
                                        <p className="text-muted small fw-bold text-uppercase mb-0 opacity-50" style={{ fontSize: '8px' }}>Min Token</p>
                                        <p className={`fw-black mb-0 ${isDarkMode ? 'text-white' : 'text-slate-900'}`} style={{ fontSize: '14px' }}>₹{course.minTokenAmount.toLocaleString()}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {courses.length === 0 && !loading && (
                        <div className="col-12 text-center py-5 opacity-50">
                            <div className="mb-3"><BookOpen size={48} strokeWidth={1} className="text-muted" /></div>
